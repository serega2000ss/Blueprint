...
class sfLuceneModelResult extends sfLuceneResult
{
  ...
  */
  public function getInternalUri()
  {
    ...
    $route = str_replace("%culture%",sfContext::getInstance()->getUser()->getCulture(), $route);
    $route_with_params = preg_replace('/%(\w+?)%/e', '$this->result->getDocument()->getFieldValue("$1")', $route);
    if (strpos($route,"@")===0) {
